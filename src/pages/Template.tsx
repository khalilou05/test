...
const TemplatePage: React.FC<TemplatePageProps> = ({ mapEditorRef }) => {
  ...
  const handleApplyTemplate = (template: any) => {
    ...
    const mergedLabels = {
      ...
      stats: Array.isArray(template.labels?.stats)
        ? template.labels.stats.map((stat: StatLabel) => ({
            label: stat.label || "",
            value: stat.value || "",
            style: stat.style || {},
          }))
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-0 w-full">
          {templates.map((template, idx) => (
            <button
              key={idx}
              type="button"
              className="group flex flex-col items-center transition-none scale-100 cursor-pointer overflow-hidden relative border-0 w-full h-[221px] bg-transparent p-0 m-0 transform hover:scale-110 transition duration-300 hover:z-10"
              style={{
                height: 221,
                background: "none",
                padding: 0,
